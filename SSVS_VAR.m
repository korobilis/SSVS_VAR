...
%      algorithm of George and McCulloch (1993), JASA, for VAR models.
%
%--------------------------------------------------------------------------
...
ylag = mlag(y,p); % Y is [T x M]. ylag is [T x (pL)]

% Insert "ylag" defined above to form X matrix (no constant)
xmat = ylag(p+1:Traw,:);

% Form y matrix accordingly
% Delete first "p" rows to match the dimensions of X matrix
ymat = y(p+1:Traw,:); % This is the final Y matrix used for the VAR

%Traw was the dimesnion of initial data. T is the number of actual 
%time series observations of ymat and xmat (final Y & X)
T=Traw-p;

n=(p*M)*M;  % n is the number of alpha parameter elements (is the number of
              % rows of the "stacked" vector of parameters)              

m = n; %choose number of restrictions
%----------------------------PRELIMINARIES---------------------------------
...
it_print = 500; % Print every "it_print"-th draw
thin = 1;   % Consider every thin-th draw (thin value)

% Set storage space in computer memory for parameter matrices
%alpha_non = zeros(n-m,nods-bid); % vector of alphas with no restrictions
alpha_draws = zeros(m,nsave); % vector of restricted alphas draws
psi_ii_sq = zeros(M,1); % vector of psi^2 drawn at each iteration
psi_ii_sq_draws = zeros(M,nsave); % vector of psi^2 draws
gammas_draws = zeros(m,nsave); % vector of gamma draws
omega_draws = zeros(.5*M*(M-1),nsave); %vector of omega draws
psi_mat_draws = zeros(M,M,nsave); % store draws of PSI matrix
alpha_mat_draws = zeros(n./M,M,nsave); % store draws of PHI matrix
...
ALPHA_OLS = inv(xmat'*xmat)*(xmat'*ymat);
SSE = (ymat - xmat*ALPHA_OLS)'*(ymat - xmat*ALPHA_OLS);
% Stack columns of PHI_M
alpha_OLS_vec = ALPHA_OLS(:);  % vector of "MLE" alphas (=vec(ALPHA_OLS))
...
for irep = 1:ntot
    ...
    D = diag(h_i'*eye(m)); % Create D. Here D=diag(h_i) will also do
    DRD = D*R*D;   % Prior covariance matrix for Phi_m
    psi_xx = kron((PSI_ALL*PSI_ALL'),(xmat'*xmat));
    Delta_alpha = inv(psi_xx + inv(DRD));

    miu_alpha = Delta_alpha*((psi_xx)*alpha_OLS_vec + (inv(DRD))*f_m);
    
    alphas = miu_alpha + chol(Delta_alpha)'*randn(n,1);
    
    alpha_mat = zeros(p*M,M);
    for nn_5 = 1:M
        nnn_1 = 1 + p*M*(nn_5-1);
        alpha_mat(:,nn_5) = alphas(nnn_1:p*M*nn_5);
    end
    ...
    %----------------------------------------------------------------------    
    for nn_6 = 1:n
            u_i1 = (1./tau_0)*exp(-0.5*(alphas(nn_6)./(tau_0))^2)*p_i;
            u_i2 = (1./tau_1)*exp(-0.5*(alphas(nn_6)./(tau_1))^2)*(1-p_i);
            gst = u_i1./(u_i1 + u_i2);
            gammas(nn_6,1) = bernoullirnd(gst);
    end
    
    % Save new Sum of Squared Errors (SSE)
    SSE = (ymat - xmat*alpha_mat)'*(ymat - xmat*alpha_mat);
